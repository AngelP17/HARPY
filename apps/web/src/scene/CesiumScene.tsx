...
export default function CesiumScene() {
  ...
  useEffect(() => {
    ...
    const osmTileUrl =
      process.env.NEXT_PUBLIC_OSM_TILE_URL || "/api/tiles/osm/";
  ...
  useEffect(() => {
    ...
    if (bloom) {
      ...
      bloom.uniforms.brightness = -0.2 + vision.bloom * 0.4;
    ...
    const modeStage = new Cesium.PostProcessStage({
      ...
      uniforms: {
        mode:
          vision.mode === "EO" ? 0.0 : vision.mode === "NVG" ? 1.0 : 2.0,
      },
    });
  ...
  useEffect(() => {
    ...
    for (const t of trackList) {
      ...
      if (!ent) {
        ...
        (ent as unknown as { position: Cesium.Cartesian3 }).position = pos;
