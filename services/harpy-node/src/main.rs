            ...
            match ev {
                ServerEvent::Tracks { tracks } => {
                    let filtered: Vec<UiTrack> = tracks
                        .into_iter()
                        .filter(|t| {
                            let layer = kind_to_layer(&t.kind);
                    ...
                    metrics::counter!("harpy_tracks_sent", filtered.len() as u64);
                ...
                ServerEvent::ProviderStatus { provider_status } => {
                    metrics::counter!("harpy_provider_status_sent", provider_status.len() as u64);
            ...
            match msg {
                Message::Text(t) => {
                    if let Ok(cm) = serde_json::from_str::<ClientMsg>(&t) {
                        match cm {
                            ClientMsg::Ping { t0 } => {
                                let _ = state_clone.tx.send(ServerEvent::Pong { t0 });
                            }
                            ClientMsg::Subscribe {
                                preset,
                                layers,
                                ...
                                if let Some(mut sub) = state_clone.subs.get_mut(&cid_clone) {
                                    sub.preset = preset;
